...
public class PassiveBot implements Player {
    ...
    public Action getAction() {
        ...
        // Performs a check action where possible
        if (prevAction.isCall() || prevAction.isCheck() || prevAction.isFold()) {
            action = Action.checkAction();
        }

        // Otherwise, performs a fold/call action at random
        else {
            Random rnd = new Random();
            boolean randomChoice = rnd.nextBoolean();
            if (randomChoice==true) {
                action = Action.callAction(gi);
            }
            else {
                action = Action.foldAction(gi);
            }
        }
