...
public final class PublicGameInfo implements GameInfo {
    ...
    // TODO: review changes made here to min-raise bets for limit games
    public void update(Action act, int s) {
        ...
        if (act.isAllInPass()) {
            ...
            potManager.addToPot(s, getMinRaise());
            getPlayer(s).update(act);
            ...
            // Limit number of raises in poker round
            if (numRaises<5) {
                potManager.addToPot(s, getMinRaise() + act.getToCall());
                getPlayer(s).update(act);
                this.numRaises++;
            ...
            // If exceeds raise limit, then force player to call
            else {
                potManager.addToPot(s, act.getToCall());
                getPlayer(s).update(act);
                // if someone calls preflop, the bigblind gets a chance to act
    ...
    private Action correctPlayerErrors(Action act, int s) {
        ...
        switch (act.getType()) {
            ...
            case Action.RAISE:
                if (numRaises>=5) {
                    return Action.callAction(getMinRaise());
                }
                if (getAmountToCall(s) <= 0.0) {
                    return Action.betAction(getMinRaise());
                } else {
                    ...
                    if ((act.getAmount() + act.getToCall()) > player.getBankRoll()) {
                        ...
                        return Action.raiseAction(getAmountToCall(s), getMinRaise());
                    }
