...
public class HandHistoryWriter implements GameObserver, HoleCardsObserver {
    private static final Logger log = Logger.getLogger(HandHistoryWriter.class);
    ...
    public void gameOverEvent() {
        ...
        if (outWriter != null) {
            try {
                ...
                log.error("error writing handhistory", e);
            }
        } else {
            log.error("no writer set, can't write HandHistory");
        }
    ...
    public void stageEvent(int stage) {
        for (int i = 0; i < gameInfo.getNumSeats(); i++) {
            amountInPotThisRound.put(i, (double) 0);
        }
    ...
    private void writeInitalGameInfo() {
        ...
        if (gameInfo.isActive(9)) {
            log.warn("Full-Tilt format only supports 9 players, but 10 players are seated");
        }
    ...
    static class ShowdownDataCache {
        ...
        public boolean getAmountFromPot(int pos, double amount) {
            ...
            // if the amount is smaller than the pot's amount, this pot is marked
            // as tied
            if (amount < currentSidePotAmount) {
            ...
            if (currentSidePotAmount < 0.001) {
                ...
                potsAtShowDown.set(currentPot, currentSidePotAmount);
            }
