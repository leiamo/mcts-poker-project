...
/** 
 * A Simple example bot based on the 'SimpleBot.csv' embedded with the Meerkat-API.<br>
 ...
 * <br>
 * Some tests show that SimpleBot.csv beats default XenBot from Poker-Academy with 40bb/100 but loses agains
 ...
 * (This doesn't necessarily imply SimpleBot.csv is 'better' (against real humans), as this bot
...
public class SimpleBot implements Player {
	...
	public Action getAction() {
		System.out.println("\tYour Hand: [" + c1.toString() + "-" + c2.toString() + "]");
		Action botMove;
		if (gi.isPreFlop()) {
			botMove =  preFlopAction();
		} else {
			botMove = postFlopAction();
		}
		System.out.println("\tYour move: " + botMove.toString());
		return botMove;
	}
